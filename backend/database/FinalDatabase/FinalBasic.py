...
def final_basic_schema() -> str:
    ...
    for column in basic_table_elements:
        add_comma = ", "
        ...
        default = ""
        add_quotes = ""
        if basic_table_elements[column]["Type"] == "Number":
            if basic_table_elements[column]["Decimal"] == 0:
                column_type = " INTEGER"
            else:
                column_type = " REAL"
        else:
            column_type = " TEXT"
            add_quotes = "'"
        if basic_table_elements[column]["Importance"] == "mandatory for sheet":
            required = " NOT NULL"
        if basic_table_elements[column]["default"] != "":
            default = " DEFAULT " + f"{add_quotes + basic_table_elements[column]['default'] + add_quotes}"
